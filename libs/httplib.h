...
template <typename T>
inline T get_header_value(const Headers & /*libs*/,
...
inline bool read_headers(Stream &strm, Headers &headers) {
  ...
  for (;;) {
    ...
    if (line_reader.end_with_crlf()) {
      // Blank line indicates end of libs.
  ...
  if ((res.status == 401 || res.status == 407) &&
    ...
    if (!username.empty() && !password.empty()) {
      ...
      if (detail::parse_www_authenticate(res, auth, is_proxy)) {
        ...
        new_req.libs.erase(is_proxy ? "Proxy-Authorization"
                                       : "Authorization");
        new_req.libs.insert(detail::make_digest_authentication_header(
...
#ifdef CPPHTTPLIB_ZLIB_SUPPORT
  if (compress_) { req.libs.emplace("Content-Encoding", "gzip"); }
#endif
      ...
      if (detail::parse_www_authenticate(res2, auth, true)) {
        ...
        if (!detail::process_client_socket(
                ...
                write_timeout_sec_, write_timeout_usec_, [&](Stream &strm) {
                  ...
                  req3.libs.insert(detail::make_digest_authentication_header(
